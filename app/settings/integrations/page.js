      ...
      {ghlConn?.connected ? (
        ...
        <Card className="bg-[#161630] border-[#2A2A55] p-6">
          ...
          <div className="space-y-2">
            <Input type="password" aria-label="GoHighLevel API key" placeholder="API key" value={ghl.apiKey} onChange={(e) => setGhl({ ...ghl, apiKey: e.target.value })} className="bg-[#0F0F26] border-[#2A2A55]" />
            <Input aria-label="GoHighLevel Location ID" placeholder="Location ID" value={ghl.locationId} onChange={(e) => setGhl({ ...ghl, locationId: e.target.value })} className="bg-[#0F0F26] border-[#2A2A55]" />
