...
// failOpen: an Upstash outage shouldn't take down the app. The exception is
// anonymous LLM traffic, where "allow" means unattributable paid inference —
// see checkLlmRateLimit.
async function hit(key, limit, { failOpen = true } = {}) {
  if (!redis) return hitMemory(key, limit)
  try {
    const { success } = await getLimiter(limit).limit(key)
    return success
  } catch (err) {
    console.error(`rateLimit: Upstash check failed for ${key}, ${failOpen ? 'allowing' : 'denying'} request:`, err.message)
    return failOpen
  }
}
...
//
// The anonymous path fails CLOSED. Both of its caps used to fail open at once,
// so a simultaneous Upstash and Firestore wobble left anonymous Gemini spend
// with no durable ceiling at all. A 429 on the public demo is cheaper than an
// unbounded bill — and Firestore being down means the demo is broken anyway,
// since every route on it reads the same database.
export async function checkLlmRateLimit(request, uid) {
  if (uid) return hit(`llm:uid:${uid}`, LLM_AUTH_PER_MIN)

  const ip = clientIp(request)
  if (!(await hit(`llm:ip:${ip}`, LLM_UNAUTH_PER_MIN, { failOpen: false }))) return false
  ...
  try {
    ...
    // Fail closed: this counter is the only durable ceiling on anonymous paid
    // inference. The minute bucket resets on every cold start and doesn't span
    // instances, so it is not a backstop.
    console.error('llm daily cap check failed, denying anonymous request:', err.message)
    return false
  }
}
