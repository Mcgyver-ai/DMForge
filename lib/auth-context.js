...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, u => {
      setUser(u)
      setLoading(false)
      // ponytail: presence-only cookie — not JWT-verified at edge. Real auth enforced
      // in every API route via Firebase Admin token check. This is a redirect hint only.
      if (u) {
        document.cookie = 'dmforge-auth=1; path=/; SameSite=Lax; max-age=2592000'
      } else {
        document.cookie = 'dmforge-auth=; path=/; max-age=0'
      }
    })
    return () => unsub()
  }, [])
