...
// can route through any Gemini-compatible proxy instead of Google directly:
//  - Cloudflare AI Gateway (logs/caching/analytics):
//    GEMINI_BASE_URL=https://gateway.ai.cloudflare.com/v1/{account_id}/dmforge/google-ai-studio
//  - local CLIProxyAPI: GEMINI_BASE_URL=http://127.0.0.1:8317
// Defaults to Google. The API key travels in the x-goog-api-key header (never
// the URL) so proxies/gateways don't log it.
const GEMINI_BASE = () => (process.env.GEMINI_BASE_URL || 'https://generativelanguage.googleapis.com').replace(/\/+$/, '')
const GEMINI_URL = (model) => `${GEMINI_BASE()}/v1beta/models/${model}:generateContent`
...
// `response_schema` is Gemini's own structured-output contract (uppercase type
// enum: OBJECT / STRING / ARRAY / BOOLEAN / INTEGER). Passing one makes the
// model return a shape rather than prose we have to pick apart afterwards.
// `thinking_budget` caps Gemini 2.5's reasoning tokens, which are billed against
// maxOutputTokens. 0 disables thinking — necessary for short structured replies,
// where reasoning can eat the whole budget and truncate the JSON mid-string.
// (Gemini 3.x renames this to thinkingLevel; MODEL is 2.5, so budget it is.)
export async function chat({ messages, model = MODEL, temperature = 0.7, max_tokens = 1500, response_format = null, response_schema = null, thinking_budget = null }) {
...
export async function chatJSON({ messages, model = MODEL, temperature = 0.5, max_tokens = 4000, response_schema = null, thinking_budget = null }) {
